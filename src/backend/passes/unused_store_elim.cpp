#include "backend/passes/unused_store_elim.h"
...
void unused_store_elim_function(FunctionPtr function, Builder &builder)
{
    ...
    while (curr_bb != function->tail_basic_block)
    {
        ...
        while (curr_instr != curr_bb->tail_instruction)
        {
            ...
            if (curr_instr->is_load())
            {
                ...
                auto load_op = curr_instr->as<instruction::Load>()->op;
                auto load_rd_id = curr_instr->as<instruction::Load>()->rd_id;
                auto load_rd = builder.context.get_operand(load_rd_id);
                auto load_rs_id = curr_instr->as<instruction::Load>()->rs_id;
                auto load_rs = builder.context.get_operand(load_rs_id);
                auto load_imm_id = curr_instr->as<instruction::Load>()->imm_id;
                ...
                if (load_op == instruction::Load::Op::LW && load_rs->is_sp())
                {
            ...
            {
                auto binary_rs_id = curr_instr->as<instruction::BinaryImmediate>()->rs_id;
                auto binary_rs = builder.context.get_operand(binary_rs_id);
                auto binary_rd_id = curr_instr->as<instruction::BinaryImmediate>()->rd_id;
            ...
            {
                auto binary_rs1_id = curr_instr->as<instruction::Binary>()->rs1_id;
                auto binary_rs1 = builder.context.get_operand(binary_rs1_id);
                auto binary_rs2_id = curr_instr->as<instruction::Binary>()->rs2_id;
    ...
    while (curr_bb != function->tail_basic_block)
    {
        ...
        while (curr_instr != curr_bb->tail_instruction)
        {
            ...
            {
                auto store_op = curr_instr->as<instruction::Store>()->op;
                auto store_rs1_id = curr_instr->as<instruction::Store>()->rs1_id;
                auto store_rs1 = builder.context.get_operand(store_rs1_id);
                auto store_rs2_id = curr_instr->as<instruction::Store>()->rs2_id;
                auto store_rs2 = builder.context.get_operand(store_rs2_id);
                auto store_imm_id = curr_instr->as<instruction::Store>()->imm_id;
                ...
                if (store_op == instruction::Store::Op::SW && store_rs1->is_sp())
                {
